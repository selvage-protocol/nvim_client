 ...
 * `../reference_server/target/{debug,release}/selvaged`. A missing binary fails the run with
 * the command that produces it rather than skipping: the point of the e2e harness is that
 * every process in it is real.
 */
...
export class RealServer {
  ...
  private constructor(child: ChildProcessByStdio<null, Readable, Readable>, address: string) {
    this.child = child;
    this.address = address;
    this.wsBase = `ws://${address}`;
  }

  /** Starts a server on an ephemeral loopback port and reports the address it says it is on. */
  static async start(): Promise<RealServer> {
    ...
    return new Promise<RealServer>((resolve_, reject) => {
      ...
      child.stdout.on('data', (chunk: Buffer) => {
        ...
        if (match !== null) {
          clearTimeout(timer);
          resolve_(new RealServer(child, match[1] as string));
        }
      });
